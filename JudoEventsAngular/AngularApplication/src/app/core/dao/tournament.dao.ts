...
export class TournamentDao{
  ...
  public fetchAllTournaments(): Promise<Event[]>{
    return this.http.get<Event[]>(this.appConfig.serverURL+'/tournaments').toPromise();
  }

  public async createTournament(tournament: Event):Promise<void>{
    let json: any = tournament;

    await this.http.post<void>(this.appConfig.serverURL+'event',tournament).toPromise();
  }
